...
Spectrum path_tracing(const Scene &scene,
    ...
    for (int num_vertices = 3; max_depth == -1 || num_vertices <= max_depth + 1; num_vertices++) {
        ...
        // BSSRDF
        if (std::holds_alternative<BSSRDF>(mat)) {
            // Now we only consider direct lighting.

            // BSSRDF part
            const BSSRDF& bssrdf = std::get<BSSRDF>(scene.materials[vertex.material_id]);
            Vector3 wo = -ray.dir;

            Spectrum contrib = L_bssrdf(scene, vertex, bssrdf, wo, rng);
            radiance += current_path_throughput * contrib;


#if BSSRDF_SPEC == 0
            /*
            For BSSRDF only, End path propagation here
            */
            break;
#endif
            ...
            std::optional<PathVertex> specular_vertex = intersect(scene, ray);
            if (specular_vertex &&  is_light(scene.shapes[specular_vertex.value().shape_id])) {
                // Hit light, I terminate here
                radiance += current_path_throughput * emission(*specular_vertex, -ray.dir, scene) ;
                break;
            }
            if (!specular_vertex && has_envmap(scene)) {
